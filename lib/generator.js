...
exports.generateData = function (files, options, callback) {
	...
	formats.forEach(function(format, i){
		...
		options.files.forEach(function (item, i) {
			...
			var animationFrameMatch = item.name.match(/^(.+)_(\d+)$/);
			if (animationFrameMatch) {
				var name = animationFrameMatch[1];
				var number = parseInt(animationFrameMatch[2]);

				var ani = animations[name];
				if (ani) {
					ani.frames.push({
						name: item.name.concat(item.extension),
						number,
						isLast: false,
					});
				} else {
					newAni = {
						...
						isLast: options.animations.length === 0
					}
					animations[name] = newAni;
					options.animations.splice(0, 0, newAni);
				}
			}
		});

		options.animations.forEach(function (ani) {
			ani.frames.sort(function (a, b) {
				return a.number > b.number;
			});
			ani.frames[ani.frames.length - 1].isLast = true;
		});
